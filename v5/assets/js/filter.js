...
function renderMenuItems(section, items) {
    ...
    items.forEach(item => {
        ...
        dishDiv.setAttribute('data-kind', item.kind);
        ...
        dishDiv.innerHTML = `
            ...
            <button data-name="${item.name}" data-price="${item.price}">Добавить</button>
        `;

        menuGrid.appendChild(dishDiv);
    });
}
...
Object.keys(sections).forEach(category => {
    const sectionItems = menuItems.filter(item => item.category === category);
    renderMenuItems(sections[category], sectionItems);
});
...
function filterDishes() {
    ...
    const category = parentSection.id;

    parentSection.querySelectorAll('.filter-buttons button').forEach(btn => btn.classList.remove('active'));
    this.classList.add('active');

    const filteredItems = menuItems.filter(item =>
        item.category === category && (kind === 'all' || item.kind === kind)
    );

    renderMenuItems(parentSection, filteredItems);
}
